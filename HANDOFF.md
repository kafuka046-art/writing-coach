...
单文件 AI 英语写作助手（入库学 → 仿写练 → 出库用 → 对错回流 + 雅思 5/6/7 对标）。
...
├── tests\                    # 回归测试：node tests\closed-loop.test.js
...
- [x] prompt 改造：固定 8 段 → 自适应 3-6 段（去重、不凑段落、400 字上限、"该句式等级"、"现在轮到你了"仿写提示行）
- [x] 仿写闭环：讲解卡下方练习输入框（AI 判断用没用对 → 对=已掌握 / 错=纠错；同句式只留一条记录，练对自动消解旧纠错）
- [ ] 信息架构 + 视觉重设计（方案见 docs\设计方案_v1.md；主色保留蓝，弃用"入库/出库"改为"学会/写/复习"）
...
- [x] 纠错条目消解（限仿写闭环：同句式再练对，来源为「仿写闭环」的纠错自动翻成「已掌握」；作文检查产生的纠错仍不消解）
- [ ] 流式输出（SSE）+ 请求超时/重试/取消
...
2. 作文检查产生的纠错条目仍不消解（仿写闭环内的已能消解）；知识点仍用字符串 `"ERROR:"` 传递错误，健壮性一般
3. 无流式输出、无超时重试；`topicImages` 刷新即丢（草稿会存、图片不存）
4. dist/ 与 zip 由根目录重新打包；改代码后记得重打包
5. 手机端只做了布局适配，未真机验证
6. 无 Key 时仿写判定走正则匹配，句式变体识别不出（如写了 not only 但省略 but also，会判成"没出现目标句式"）