...
- 🖼️ **题目图片**：Ctrl+V 粘贴或上传，AI 读图后判断作文是否切题
...
- **本地代理的安全处理**：仅绑定 127.0.0.1、Host 头校验（防 DNS rebinding）、请求体 12MB 上限、路径穿越防护
...
- [x] 浏览器直连（去掉线上对后端的依赖）
- [x] 移动端布局适配（需真机复查）
- [x] 题目图片送入多模态模型（V4.1 Flash 读图 → 概括题目 + 判断作文是否切题；图片自动压缩到 ≤1440px）
